...
export class Command {
    ...
    setupProgram() {
        ...
        // Start command - session management wrapper
        this.program
            .command('start')
            .description(i18n.t('commands.start'))
            .argument('[session]', 'Session name to start')
            .action(async (session) => {
                await startCommand(session ? [session] : []);
            });

        // Session management commands
        registerSessionCommands(this.program);
