...
export class Wizard {
    ...
    printBanner() {
        ...
        const titleLine = createContentLine('Coding Helper v0.0.5', '║', '║', BANNER_WIDTH, 'center');
        const asciiLines = [
            ' ▄▀▀ ▄▀▄ █▀▄ █ █▄ █ ▄▀    █▄█ ██▀ █   █▀▄ ██▀ █▀▄ ',
            ' ▀▄▄ ▀▄▀ █▄▀ █ █ ▀█ ▀▄█   █ █ █▄▄ █▄▄ █▀  █▄▄ █▀▄ '
    ...
    async configUrl() {
        while (true) {
            this.resetScreen();
            this.createBox(i18n.t('wizard.config_url'));
            const currentConfig = configManager.getConfig();
            if (currentConfig.url) {
                console.log(chalk.gray('  ' + i18n.t('wizard.config_url') + ': ') + chalk.gray(currentConfig.url));
                console.log('');
            }
            const { action } = await this.promptWithHints([
                {
                    ...
                    choices: [
                        { name: '>   ' + (currentConfig.url ? i18n.t("wizard.update_url") : i18n.t('wizard.input_url')), value: 'input' },
                        new inquirer.Separator(),
                        { name: '<-  ' + i18n.t('wizard.nav_return'), value: 'back' },
                        { name: 'x   ' + i18n.t('wizard.nav_exit'), value: 'exit' }
                    ]
                ...
                const { url } = await inquirer.prompt([
                    {
                        type: 'input',
                        name: 'url',
                        message: i18n.t('wizard.input_your_url'),
                        default: currentConfig.url || '',
                        validate: (input) => {
                            if (!input || input.trim().length === 0) {
                                return '[!] ' + i18n.t('wizard.url_required');
                            }
                            try {
                                new URL(input);
                                return true;
                                ...
                                return '[!] ' + i18n.t('wizard.url_invalid');
                            }
                        }
                    }
                ]);
                configManager.setUrl(url.trim());
                console.log(chalk.green("✅ " + i18n.t('wizard.set_success')));
    ...
    async configureTool(toolName) {
        ...
        // 自动应用配置到工具
        await this.applyConfigToTool(toolName);
    }
    async applyConfigToTool(toolName) {
        const config = configManager.getConfig();
        if (!config.url || !config.api_key) {
            console.log(chalk.red('\n[!] ' + i18n.t('wizard.missing_config_url_or_key')));
            await new Promise(resolve => setTimeout(resolve, 1500));
            return;
        }
        const spinner = ora({
            text: i18n.t('wizard.applying_config', { tool: SUPPORTED_TOOLS[toolName].displayName }),
            spinner: 'star2'
        }).start();
        try {
            // 应用配置到工具
            toolManager.applyConfig(toolName, config.url, config.api_key);
            await new Promise(resolve => setTimeout(resolve, 800));
            spinner.succeed(chalk.green(i18n.t('wizard.config_applied_success', { tool: SUPPORTED_TOOLS[toolName].displayName })));
            console.log('');
            console.log(chalk.cyan.bold(i18n.t('wizard.config_details') + ':'));
            console.log(chalk.gray('  ' + i18n.t('wizard.config_url') + ': ') + chalk.white(config.url));
            console.log(chalk.gray('  ' + i18n.t('wizard.config_api_key') + ': ') + chalk.gray(i18n.t('wizard.api_key_set') + ' (' + config.api_key.slice(0, 4) + '****)'));
            console.log('');
            console.log(chalk.green('✅ ' + i18n.t('wizard.ready_to_use', { tool: SUPPORTED_TOOLS[toolName].displayName, command: SUPPORTED_TOOLS[toolName].command })));
            await new Promise(resolve => setTimeout(resolve, 2000));
        }
        catch (error) {
            logger.logError('Wizard.applyConfigToTool', error);
            spinner.fail(i18n.t('wizard.config_apply_failed'));
            await new Promise(resolve => setTimeout(resolve, 800));
            console.error(error);
        }
    }
    async showMainMenu() {
        ...
        while (true) {
            ...
            // 显示当前配置状态
            console.log(chalk.gray('  ' + i18n.t('wizard.config_url') + ': ') + (currentCfg.url ? chalk.gray(currentCfg.url) : chalk.red(i18n.t('wizard.not_set'))));
            ...
            const choices = [
                ...
                { name: '>   ' + i18n.t('wizard.menu_config_url'), value: 'url' },
