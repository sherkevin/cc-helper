...
export class ToolManager {
    ...
    applyConfig(toolName, url, apiKey) {
        const tool = SUPPORTED_TOOLS[toolName];
        if (!tool) {
            throw new Error(`Unknown tool: ${toolName}`);
        }
        // Claude Code 使用专门的管理器
        if (toolName === 'claude-code') {
            claudeCodeManager.applyConfig(url, apiKey);
            return;
        }
        // OpenCode 使用专门的管理器
        if (toolName === 'opencode') {
            openCodeManager.applyConfig(url, apiKey);
            return;
        }
        // Crush 使用专门的管理器
        if (toolName === 'crush') {
            crushManager.applyConfig(url, apiKey);
            return;
        }
        // Factory Droid 使用专门的管理器
        if (toolName === 'factory-droid') {
            factoryDroidManager.applyConfig(url, apiKey);
            return;
        }
        // 通用工具配置
        let config = this.getToolConfig(toolName) || {};
